...
上一章我们学过 Q-Learning 在 GridWorld 上跑得很好——16 个格子、4 个方向、64 个 $Q$ 值，一张小表格就搞定了。你可能还记得，$Q(s,a)$ 是动作价值函数，回答的是"在状态 $s$ 下先做动作 $a$，之后按策略行动，能拿多少分"；TD 方法则让我们不用等一整局结束，走一步就能更新预测。Q-Learning 把两者结合起来：每走一步，用"实际奖励 + 下一状态的最优价值估计"来修正当前 $Q$ 值。但现实世界远不是一张 4×4 的棋盘：CartPole 的状态是连续的 4 维向量，Atari 游戏的一帧画面有 28,000 多个像素值。Q-Learning 的更新规则本身没有问题，问题出在"用表格存 Q 值"这件事上——状态太多，表格装不下。要理解为什么需要深度 Q 网络，首先要理解表格方法在哪些地方撞墙，最后才能明白"用神经网络替代表格"不是一个可有可无的优化，而是一道必须跨过去的坎。
