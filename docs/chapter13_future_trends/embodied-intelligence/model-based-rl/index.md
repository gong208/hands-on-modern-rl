...
数学上，Model-Free 并不是“不存在环境转移”，而是 **不去显式学习这个转移模型**。环境真实转移 $p(s_{t+1}\mid s_t,a_t)$ 只负责产生采样轨迹：
...
如果走价值函数路线，例如 DQN、SAC、TD3，核心是用真实采样到的 $s_{t+1}$ 做 TD bootstrapping：
...
注意这里的 $s_{t+1}$ 来自真实环境或仿真器记录下来的 replay buffer，**不是** 神经网络世界模型生成出来的。
...
所以两者最关键的数学差异是：**Model-Free 的学习信号来自真实采样的 $s_{t+1}$；Model-Based 先学习 $\hat{p}_\phi$，再用 $\hat{s}_{t+1}$ 和 $\hat{r}_t$ 做规划或训练。**
