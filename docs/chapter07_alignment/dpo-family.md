...
## Self-Rewarding & Self-Critic：让模型自己当裁判

前面所有方法——无论 DPO、KTO、SimPO 还是 IPO——都隐含了一个前提：**需要一个外部信号源**来告诉模型什么是好的、什么是坏的。偏好数据来自人类标注，👍/👎来自用户反馈，Reference Model 来自训练前的模型快照。但这些外部信号都有代价：人类标注昂贵且慢，用户反馈噪声高，Reference Model 只是一个静态的基线。

Self-Rewarding 和 Self-Critic 提出了一个更激进的思路：**让模型自己生成奖励信号**。这不是"绕过 RM"（DPO 已经做到了），而是"根本不需要 RM"——模型就是自己的裁判。

### Self-Rewarding：模型给自己打分

Self-Rewarding LMs（Meta, 2024）的核心流程如下：

1. 模型对同一个 prompt 生成多个回答
2. 模型自己给每个回答打分（LLM-as-Judge）
3. 选择最高分和最低分的回答构造偏好对
4. 用 DPO 在这些自生成的偏好对上训练

```python
# ==========================================
# Self-Rewarding：模型自生成偏好数据
# ==========================================

def self_reward_pipeline(model, prompt, num_samples=4):
    """模型自己生成回答并自己评分"""
    # 1. 生成多个候选回答
    ...
    for resp in responses:
        judge_prompt = f"请对以下回答的质量打分（1-10分）：\n问题: {prompt}\n回答: {resp}\n分数:"
        score = model.generate(judge_prompt)
...
Self-Rewarding 的妙处在于它可以**迭代**——训练一轮后，新模型的自评能力更强，生成的偏好数据质量更高，再训练下一轮。Meta 的实验表明，经过 3 轮迭代后，模型在 AlpacaEval 上的胜率从基础模型的 10% 提升到 73%。

### Self-Critic：模型反思并修正

Self-Critic 走的是另一条路：不直接打分，而是让模型**自我批评并修正**。流程是：

1. 模型生成一个回答
2. 模型扮演"批评者"，指出回答中的问题
3. 模型扮演"修订者"，根据批评改进回答
4. 原始回答 = rejected，修订后回答 = chosen

这和 Constitutional AI（第 10 章会详细讲）的思路一脉相承，但 Self-Critic 的批评原则更加灵活——可以是通用的"质量评估"，也可以是特定领域的"数学推理检查"或"代码逻辑审查"。

### SPPO：Self-Play + 偏好优化

SPPO（Self-Play Preference Optimization）把 Self-Rewarding 和博弈论结合起来：模型既当"选手"（生成回答），又当"裁判"（评估回答），同时和一个不断升级的"过去版本"竞争。每一轮训练后，新的模型要超越上一轮的自己。

### 与其他方法的关系

Self-Rewarding / Self-Critic 和本章其他方法的定位不同：

| 方法               | 外部信号源     | 能否迭代 | 核心风险         |
| ------------------ | -------------- | -------- | ---------------- |
| DPO                | 人类偏好对     | 否       | 受限于数据质量   |
| KTO                | 用户👍/👎      | 否       | 信号弱           |
| SimPO              | 无（自身概率） | 否       | 缺乏 KL 约束     |
| **Self-Rewarding** | **模型自评分** | **能**   | **自我确认偏见** |
| **Self-Critic**    | **模型自批评** | **能**   | **批评质量不足** |

Self-Rewarding 最大的风险是"自我确认偏见"（Self-Confirmation Bias）——模型可能给自己擅长但不好的回答打高分，然后在这些回答上训练，进一步强化这种偏好。缓解策略包括：定期用外部基准测试校准、引入多样性约束、以及混合使用人类标注数据。
...
| 无外部标注，想自我迭代          | **Self-Rewarding** | 模型自评自训，可多轮迭代                 |
| 需要精细的质量改进              | **Self-Critic**    | 批评→修订→再训练，迭代提升回答质量       |
...
print("  - 没有标注数据但想迭代？用 Self-Rewarding")
```
...
flowchart TD
    ...
    style SelfR fill:#fff8e1,stroke:#f9a825
```
...
- DPO → Self-Rewarding：去掉了外部信号源，模型自评自训
