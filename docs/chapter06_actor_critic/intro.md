...
上一章末尾我们发现了一个关键线索：减掉基线可以降低方差（回顾：[Baseline 实验](../chapter05_policy_gradient/baseline-experiment)），而最好的基线就是 $V(s)$（回顾：[状态价值函数](../chapter03_mdp/value-bellman)）。但 $V(s)$ 本身也需要学习——需要一个专门的网络来估计它。这个网络就是 **Critic**。
